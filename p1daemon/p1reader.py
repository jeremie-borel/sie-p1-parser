...
class SieP1Reader:
    ...
    def _get_frame(self) -> Generator[bytes, None, None]:
        serial_socket = serial.Serial(
            self.tty, baudrate=115200, bytesize=8, parity="N", stopbits=1, timeout=3.5
        )
        with serial_socket:
            while True:
    ...
    def read(self) -> Generator[tuple[datetime.datetime, Any], None, None]:
